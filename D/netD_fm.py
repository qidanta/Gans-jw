...
class _netDFM(nn.Module):
    ...
    def forward(self, input = 0, condition = 0):
        ...
        if self.type == 'dcgans_nob':
            ...
            z = self.sub3(z)
            self.fm.append(z)
            logints_out = self.remain(z)
            return torch.sigmoid(logints_out), logints_out, self.fm
...
def build_netDFM(cfg_index, z_dim = 0, c_dim = 0, batch_norm=False, gans_type='dcgans'):
    '''build sigle netD
    @Params:
    ...
    for index in cfg_index:
        network, z_dim = create_convnets_DFM(netDNumConfig[index], z_dim, c_dim)
        networks.append(network)
    return _netDFM(layers=networks, type=gans_type)


if __name__ == '__main__':
    netD = build_netDFM(['dcgans_nob_sub1', 'dcgans_nob_sub2', 'dcgans_nob_sub3', 'dcgans_nob_sub4', 'dcgans_logint'], z_dim=1, c_dim = 0, gans_type='dcgans_nob')
    print netD