...
from CG.netCG import build_netCG
...
class _competitionGan(_baseModel):
    ...
    def backward_G(self):
        ...
        self.loss_G_l1 = self.criterionL1(self.fake[index], self.X)

        preb_G_gap = []
        self.label.data.fill_(1)
        for i in range(self.nums):
            if i != index:
                gap = self.criterionGAN(D_fake[i], self.label)# * 0.1
                preb_G_gap.append(gap)
        self.loss_G_lambda = sum(preb_G_gap)

        #self.loss_G = self.loss_G_fake_like + self.loss_G_l1 + self.loss_G_lambda
    ...
    def save_network(self, it, savepath):
        print 'training-result-netGPth: iters_{}netCG.pth/index_{}/storing'.format(it, self.best_netG_index)
        torch.save(self.netCG.state_dict(), '{}/netCG_epoch_{}_index_{}.pth' .format(savepath, it, self.best_netG_index))
    ...
    def save_image(self, fake, it , savepath):
        ...
        if self.train:
            print 'training-result-imgs: iters_{}/index_{}/storing'.format(it, self.best_netG_index)
            plt.savefig(self.savepath+ '/{}_{}.png'.format(str(it), self.best_netG_index), bbox_inches='tight')
        else:
            print 'testing-result-imgs: iters_{}'.format(it)
            plt.savefig(self.savepath+ '/{}.png'.format(str(it)), bbox_inches='tight')

    def store(self, epoch):
        print 'store: [{}/{}]'.format(epoch, self.opt.niter * 60000)
        print '==========\n'
        if not os.path.exists(self.savepath):
            os.makedirs(self.savepath)
