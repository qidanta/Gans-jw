...
class _competitionGan(_baseModel):
    def __init__(self, opt):
        ...
        Z = torch.FloatTensor(opt.mb_size, opt.z_dim, opt.img_size, opt.img_size)
    ...
    def draft_data(self, input):
        real_input = input['A']
        real_output = input['B']
        self.mb_size = real_input.size(0)
        self.X.data.resize_(real_output.size()).copy_(real_output)
        self.X.data.resize_(self.mb_size, self.x_dim, self.img_size, self.img_size)
        self.Z.data.resize_(real_input.size()).copy_(real_input)
